...
import {
  ...
  removeAllTaskValues,
  setTomValuesInStore,
} from '../../features/user/task/tomTaskSlice'
...
const Tom = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      dispatch(deleteTomTask(editId))
      const id = new Date().getTime()
      dispatch(setTomValuesInStore({ task, id, type, name }))
      dispatch(clearTomTask())
  ...
  const handleChange = (e) => {
    ...
    dispatch(getTomTaskValues({ name, value }))
  }

  // handle delete
  const handleDelete = (id) => {
    dispatch(deleteTomTask(id))
    toast.info('Task deleted...')
  }

  // handle edit
  const handleEdit = (id) => {
    dispatch(editTomTask(id))
    dispatch(toggleTaskBar())
  }
  // handle removeAllTask
  const handleRemoveAllTask = () => {
    // dispatch(removeAllTaskValues())
    dispatch(tomOpenModal())
  }
  ...
  return (
    <Wrapper>
      ...
      <h5 className='title'>Task given by Tom</h5>
      <div className='title-underline'></div>
      <hr />
      {localTomTask.length > 0 && (
        <div className='title'>
          <button
