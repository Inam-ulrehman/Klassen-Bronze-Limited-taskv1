...
} from '../../features/user/task/wendyTaskSlice'
...
const Wendy = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      dispatch(deleteWendyTask(editId))
      const id = new Date().getTime()
      dispatch(setWendyValuesInStore({ task, id, type, name }))
      dispatch(clearWendyTask())
  ...
  const handleChange = (e) => {
    ...
    dispatch(getWendyTaskValues({ name, value }))
  }

  // handle delete
  const handleDelete = (id) => {
    dispatch(deleteWendyTask(id))
    toast.info('Task deleted...')
  }

  // handle edit
  const handleEdit = (id) => {
    dispatch(editWendyTask(id))
    dispatch(toggleTaskBar())
  }
  // handle removeAllTask
  const handleRemoveAllTask = () => {
    // dispatch(removeAllTaskValues())
    dispatch(wendyOpenModal())
  }
  ...
  return (
    <Wrapper>
      ...
      <h5 className='title'>Task given by Wendy</h5>
      <div className='title-underline'></div>
      <hr />
      {localWendyTask.length > 0 && (
        <div className='title'>
          <button
