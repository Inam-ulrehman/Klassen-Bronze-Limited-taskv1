...
} from '../../features/user/task/officeTaskSlice'
...
const Office = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      dispatch(deleteOfficeTask(editId))
      const id = new Date().getTime()
      dispatch(setOfficeValuesInStore({ task, id, type, name }))
      dispatch(clearOfficeTask())
  ...
  const handleChange = (e) => {
    ...
    dispatch(getOfficeTaskValues({ name, value }))
  }

  // handle delete
  const handleDelete = (id) => {
    dispatch(deleteOfficeTask(id))
    toast.info('Task deleted...')
  }

  // handle edit
  const handleEdit = (id) => {
    dispatch(editOfficeTask(id))
    dispatch(toggleTaskBar())
  }
  // handle removeAllTask
  const handleRemoveAllTask = () => {
    // dispatch(removeAllTaskValues())
    dispatch(officeOpenModal())
  }
  ...
  return (
    <Wrapper>
      ...
      <h5 className='title'>Task given by Office</h5>
      <div className='title-underline'></div>
      <hr />
      {localOfficeTask.length > 0 && (
        <div className='title'>
          <button
