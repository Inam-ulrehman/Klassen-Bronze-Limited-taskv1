...
} from '../../features/user/task/salesRepTaskSlice'
...
const SalesRep = () => {
  ...
  const {
    ...
    isEditing,
    isModalOpen,
  } = useSelector((state) => state.salesRep)
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      dispatch(deleteSalesRepTask(editId))
      const id = new Date().getTime()
      dispatch(setSalesRepValuesInStore({ task, id, type, name }))
      dispatch(clearSalesRepTask())
  ...
  const handleChange = (e) => {
    ...
    dispatch(getSalesRepTaskValues({ name, value }))
  }

  // handle delete
  const handleDelete = (id) => {
    dispatch(deleteSalesRepTask(id))
    toast.info('Task deleted...')
  }

  // handle edit
  const handleEdit = (id) => {
    dispatch(editSalesRepTask(id))
    dispatch(toggleTaskBar())
  }
  // handle removeAllTask
  const handleRemoveAllTask = () => {
    // dispatch(removeAllTaskValues())
    dispatch(salesRepOpenModal())
  }
  ...
  return (
    <Wrapper>
      ...
      <h5 className='title'>Task given by SalesRep</h5>
      <div className='title-underline'></div>
      <hr />
      {localSalesRepTask.length > 0 && (
        <div className='title'>
          <button
